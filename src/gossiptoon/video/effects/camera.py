...
from gossiptoon.core.constants import EffectType
...
class CameraEffect(Effect):
    ...
    def _create_delegate(self) -> Optional[Effect]:
        ...
        # Static shot
        if eff_type == EffectType.STATIC:
            return None

        # Zoom In (1.0 -> 1.0 + intensity)
        if eff_type == EffectType.ZOOM_IN:
        ...
        if eff_type == EffectType.ZOOM_OUT:
            return KenBurnsEffect(KenBurnsConfig(
        ...
        if eff_type == EffectType.PAN_LEFT:
            return KenBurnsEffect(KenBurnsConfig(
                zoom_start=pan_zoom,
                zoom_end=pan_zoom,
                pan_direction="left",
        ...
        if eff_type == EffectType.PAN_RIGHT:
            return KenBurnsEffect(KenBurnsConfig(
                zoom_start=pan_zoom,
                zoom_end=pan_zoom,
                pan_direction="right",
        ...
        if eff_type == EffectType.PAN_DOWN:
            return KenBurnsEffect(KenBurnsConfig(
                zoom_start=pan_zoom,
                zoom_end=pan_zoom,
                pan_direction="down",
